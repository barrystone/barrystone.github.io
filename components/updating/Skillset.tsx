...
const CharkmarkIcon = ({ border }: any) => (
  <svg
    className="checkmark"
...
const Skillset = ({}: SkillsetProps) => {
  const [skillSetisShow, setSkillSetIsShow] = useState<boolean>(false);

  return (
    <VisibilitySensor
      onChange={(isVisible: boolean) => {
        if (isVisible) {
          setSkillSetIsShow(true);
        }
      }}
      partialVisibility
    >
      <ul className={`skillset ${skillSetisShow ? 'skillset--show' : ''}`}>
        <li className="item">
          <CharkmarkIcon />
          <h4>🏆 &nbsp;TS </h4>
          <p>Typescript</p>
